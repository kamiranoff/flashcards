...
const ReanimatedFlip = ({ perspective = 1200, rotate = 'Y', side, front, back, style }: Props) => {
  ...
  useCode(
    () =>
      block([cond(not(eq(side, flipPosition)), [set(flipPosition, runTiming(clock, flipPosition, side))])]),
    [side],
  );
      ...
      <Animated.View
        style={[
          styles.side,
          {
            ...
            transform: [{ perspective }, { [`rotate${rotate}`]: rotateValue }, { scale: scaleFront }],
          },
        ]}>
        {front}
      </Animated.View>
